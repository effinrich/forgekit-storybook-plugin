...
```bash
npm install -D @effinrich/storyforge
```
...
```bash
npx storyforge story <path> [options]
```
...
Example:
```bash
npx storyforge story src/components/Button.tsx
npx storyforge story src/components/Button.tsx --dry-run
npx storyforge story src/components/Button.tsx --overwrite --skip-interaction-tests
```

### Generate stories for all components in a directory

```bash
npx storyforge stories <dir> [options]
```
...
Example:
```bash
npx storyforge stories src/components/
npx storyforge stories src/components/ --include-tests --overwrite
```

### Generate a Playwright component test

```bash
npx storyforge test <path> [options]
```
...
```bash
npx storyforge watch <dir> [options]
```
...
```bash
npx storyforge coverage <dir> [options]
```
